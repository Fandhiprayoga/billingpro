...
<div class="row">
  ...
  <div class="col-md-4">
    <?php if ($license->status === 'active' && activeGroupCan('licenses.revoke')): ?>
    <div class="card">
      <div class="card-body">
        <form action="<?= base_url('admin/licenses/revoke/' . $license->uuid) ?>" method="post"
              onsubmit="return confirm('Yakin ingin mencabut lisensi ini? Aksi ini tidak bisa dibatalkan.')">
          <?= csrf_field() ?>
          <button type="submit" class="btn btn-danger btn-block">
            <i class="fas fa-ban"></i> Cabut Lisensi
          </button>
        </form>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-body">
        <a href="<?= base_url('admin/licenses') ?>" class="btn btn-secondary btn-block">
